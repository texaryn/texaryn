...
describe('IdentityMap', () => {
  it('registerArray creates an empty identity list', () => {
    const map = registerArray(createIdentityMap(), key('arr'))
  ...
  describe('insertItem', () => {
    it('generates a StableItemId and inserts at index', () => {
      ...
      const r0 = insertItem(map, key('arr'), 0)
      map = r0.map
      const r1 = insertItem(map, key('arr'), 1)
      map = r1.map

      const ids = map.arrayIdentities.get(key('arr'))!
    ...
    it('inserts at the beginning', () => {
      ...
      const r0 = insertItem(map, key('arr'), 0)
      map = r0.map
      const r1 = insertItem(map, key('arr'), 0)
      map = r1.map
    ...
    it('updates reverse lookup indices', () => {
      ...
      expect(map.itemLookup.get(r0.itemId)).toEqual({ containerKey: key('arr'), index: 1 })
    })
  })
  ...
  describe('removeItem', () => {
    it('removes the item and updates indices', () => {
      ...
      const r1 = insertItem(map, key('arr'), 1)
      map = r1.map
      const r2 = insertItem(map, key('arr'), 2)
      map = r2.map

      const result = removeItem(map, key('arr'), 0)
      map = result.map

      expect(result.removedId).toBe(r0.itemId)
      expect(map.arrayIdentities.get(key('arr'))).toEqual([r1.itemId, r2.itemId])
      expect(map.itemLookup.get(r1.itemId)!.index).toBe(0)
  ...
  describe('moveItem', () => {
    it('moves an item and updates indices', () => {
      ...
      for (let i = 0; i < 3; i++) {
        const r = insertItem(map, key('arr'), i)
        map = r.map
        ids.push(r.itemId)
      }

      map = moveItem(map, key('arr'), 0, 2)
      const list = map.arrayIdentities.get(key('arr'))!
