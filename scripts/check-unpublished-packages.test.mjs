...
describe('main', () => {
  it('reads the exact versions from the four package manifests', () => {
    const root = fakeRepo({ '@texaryn/core': '1.2.3', '@texaryn/schema-json': '1.2.3', '@texaryn/react': '1.2.3', '@texaryn/react-bootstrap': '1.2.3' })
  ...
  it('returns false and logs each package when all versions exist', async () => {
    const root = fakeRepo({ '@texaryn/core': '0.1.0', '@texaryn/schema-json': '0.1.0', '@texaryn/react': '0.1.0', '@texaryn/react-bootstrap': '0.1.0' })
    const lines = []
  ...
  it('returns true when the registry answers 5xx for one package', async () => {
    const root = fakeRepo({ '@texaryn/core': '0.1.1', '@texaryn/schema-json': '0.1.1', '@texaryn/react': '0.1.1', '@texaryn/react-bootstrap': '0.1.1' })
    const lines = []
